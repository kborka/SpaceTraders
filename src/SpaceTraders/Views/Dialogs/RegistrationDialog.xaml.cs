...
public partial class RegistrationDialog
{
    ...
    public RegistrationDialog() => InitializeComponent();

    private void RegistrationDialog_OnDataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        if (e.NewValue is not RegistrationDialogViewModel registrationDialogViewModel)
        {
            return;
        }
    ...
    {
        if (e.PropertyName != nameof(RegistrationDialogViewModel.RegistrationInformation))
        {
            return;
        }
